...
for i = 1:n_rep
    ...
    [P, ~, ~, escape, extinct, t_extinct] = Dynamics(A, p_self, S0, ...
        P0, herb, n_years, c, k_c, k_h, b(l), f(l), d_Z, d_B);
...
T1.AsexualSexual = reshape(repmat(b./f, n, 1), n*length(b), 1);
T1.Run = repmat((1:n)', length(b), 1);
T1.pEscape = reshape(pEscape, n*length(b), 1);
T1.timeEscape = reshape(avgTimeEscape, n*length(b), 1);
T1.pExtinct = reshape(pExtinct, n*length(b), 1);
T1.timeExtinct = reshape(avgTimeExtinct, n*length(b), 1);
T1.pRWplant = reshape(pRWplant, n*length(b), 1);
T1.timeRWplant = reshape(avgTimeRWplant, n*length(b), 1);
T1.pRRplant = reshape(pRRplant, n*length(b), 1);
T1.timeRRplant = reshape(avgTimeRRplant, n*length(b), 1);
% Write table to text file 
writetable(T1, 'Table_MGWP_Asexual_vs_Sexual_Avg');

% Create a table for the individual runs
T2 = table;
% Assign columns to table
T2.AsexualSexual = reshape(repmat(b./f, n*n_rep, 1), n*n_rep*length(b), 1);
T2.Run = repmat((1:n*n_rep)', length(b), 1);
T2.Escaped = reshape(Escaped, n*n_rep*length(b), 1);
T2.timeEscaped = reshape(timeEscaped, n*n_rep*length(b), 1);
T2.Extinct = reshape(Extinct, n*n_rep*length(b), 1);
T2.timeExtinct = reshape(timeExtinct, n*n_rep*length(b), 1);
T2.RWplant = reshape(RWplant, n*n_rep*length(b), 1);
T2.timeRWplant = reshape(timeRWplant, n*n_rep*length(b), 1);
T2.RRplant = reshape(RRplant, n*n_rep*length(b), 1);
T2.timeRRplant = reshape(timeRRplant, n*n_rep*length(b), 1);
% Write table to text file 
writetable(T2, 'Table_MGWP_Asexual_vs_Sexual');